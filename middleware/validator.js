...
let validateBodyLop = () => {
    ...
    return [
        ...
        check('fk_giang_vien', isEmptyMessage(giangVien)).not().isEmpty(),
    ]
}
...
let validateBodySinhVien = () => {
    ...
    return [
        ...
        check('gioi_tinh', isLengthMessage(gioiTinh, 0, 50)).isLength({ min: 0, max: 10 }),
        check("gioi_tinh", 'Hãy chọn giới tính là "Nam", "Nu" hoặc "Khac"').isIn(['Nam', 'Nu', 'Khac']),
        check("email", isEmptyMessage(email)).not().isEmpty(),
        check("email", 'Hãy nhập đúng định dạng email nhé').isEmail(),
        check("fk_lop", isEmptyMessage(lop)).not().isEmpty()
    ]
}

let validateLoaiTkForUser = () => {
    const loai = "Loại"
    return [
        check('loai', isEmptyMessage(loai)).not().isEmpty(),
        check("loai", 'Hãy chọn loại tài khoản là "GV", "AD" hoặc "SV"').isIn(['GV', 'AD', 'SV']),
        check('username', isEmptyMessage('Username')).not().isEmpty(),
        check('username', isLengthMessage('Username', 6, 18)).isLength({ min: 6, max: 18 }),
        check("username", isNotUnicodeMessage("Username")).isAlphanumeric("en-US"),
        check('password', isEmptyMessage('Password')).not().isEmpty(),
        check('password', isLengthMessage('Password', 6, 18)).isLength({ min: 6, max: 18 }),
        check('fk_quyen', isEmptyMessage('Quyền ')).not().isEmpty(),

    ]
}

let validateLoaiTkForUserPut = () => {
    const loai = "Loại"
    return [
        check('loai', isEmptyMessage(loai)).not().isEmpty(),
        check("loai", 'Hãy chọn loại tài khoản là "GV", "AD" hoặc "SV"').isIn(['GV', 'AD', 'SV']),
...
let validate = {
    ...
    validateLoaiTkForUser: validateLoaiTkForUser,
    validateLoaiTkForUserPut: validateLoaiTkForUserPut
};
